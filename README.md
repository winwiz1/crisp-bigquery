...
* Pagination support with up to 1000 rows (defined by the pagination size) delivered in one request. The pagination size can be set by a user within the range of 1-1000 rows and defaults to 100 rows.<br/>
Suppose the first page of data containing a batch of rows was delivered to the browser. The user can see the total number of rows in the dataset and estimate the number of pages required to paginate through it. Then start the pagination by requesting several pages of data and possibly opt to submit a more restrictive query.
...
## Getting Started
You will need NodeJS and Google Cloud SDK installed. The alternative is to use Google Cloud Shell which is a free Linux VM with both components pre-installed.  If you choose to use Cloud Shell then skip the step 1. If you prefer not to then on the subsequent steps do not open Cloud Shell and execute `bq` commands in your local environment.
  
1. **Install NodeJS and Google Cloud SDK.**<br/>
Download and run a pre-built [installer](https://nodejs.org/en/download/). Then install yarn: `npm install yarn -g`

The SDK installer can be found here.

2. **Create Google Cloud Platform (GCP) account and project.**<br/>
...
Go to GCP [API Dashboard](https://console.cloud.google.com/apis/dashboard) and ensure the project created at the previous step is selected. Then click on "+ENABLE API AND SERVICES" button at the top of the page. Choose BigQuery API on the subsequent "Welcome to the API Library" page and enable it.
...
* Grant the `bigquery.dataViewer` role to the service account by modifying the last command. Then proceed to the next step. Not recommended unless you are using a throw-away project due to granting permissions to view all project datasets.
...
To clone the repository to your workstation or Cloud Shell execute:
    ...
    >If you use Cloud Shell, git will ask for the GitHub credentials. Instead of providing the password you can use your GitHub account settings to create a personal access token and delete it later. To use the token modify the command: `git clone https://<github-username>:<token>@github.com/winwiz1/crisp-bigquery.git`.

...
2. Presentation.<br/>You can embed BigQuery data into an existing website. Implement charts, dashboards etc. using popular libraries like [Rechart](http://recharts.org/en-US/examples) without customisation restrictions imposed by pre-built dashboards. This scenario assumes the initial data-fetching delay of few seconds is acceptable. Pagination through the data fetched by already submitted query typically results in less than a second delay though your mileage can vary.
